...
interface QueuedInbound { msg: ChannelMessage; queuedAt: number; }
...
export function queueMidTurnInbound(channelId: string, msg: ChannelMessage): number {
...
export function popOldestQueuedInbound(): { channelId: string; msg: ChannelMessage } | null {
  let oldest: { channelId: string; msg: ChannelMessage; queuedAt: number } | null = null;
  for (const [channelId, q] of midTurnQueues) {
    const head = q[0];
    if (!head) continue;
    if (!oldest || head.queuedAt < oldest.queuedAt) oldest = { channelId, msg: head.msg, queuedAt: head.queuedAt };
  }
...
export function matchCommand(body: string): { name: string; arg?: string } | null {
  const m = body.match(/^(?:\/(stop|help|btw|status|reset|verbose|compact|model|jobs|todos|undo|redo)(?:\s+([\s\S]+))?|stop)$/i);
  if (!m) return null;
...
export function parseJobsFromPs(psOut: string): { profile: string; age: string; task: string }[] {
  const out: { profile: string; age: string; task: string }[] = [];
  for (const line of psOut.split("\n")) {
    const m = line.trim().match(/^(\S+)\s+\S*bash\s+\S*scripts\/pi-bg\s+(worker|reviewer)\s+(.+)$/);
    if (!m) continue;
    const task = m[3].trim().replace(/^"+|"+$/g, "").split("\n")[0].slice(0, 70);
...
export function listInflightJobs(): string {
  ...
  try {
    ...
    raw = execSync(`ps ${uid !== undefined ? `-u ${uid}` : "-eo"} -o etime,args | grep '[s]cripts/pi-bg'`, { encoding: "utf8", timeout: 5000 });
  } catch {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("turn_start", async (_event, ctx) => {
    ...
    if (!runOpen) {
      ...
      try { undoRun = startRun(ctx.cwd); } catch (e) { undoRun = null; console.error("[undo] start failed:", sanitizeUnknownValue(e)); }
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    if (queued) {
      console.log(`[channel] re-wake: starting run for queued mid-turn inbound (${queued.channelId})`);
      try { await handleInbound(pi, queued.msg, ctx, true); }
      catch (e) { console.error("[channel] re-wake failed:", sanitizeUnknownValue(e)); }
    }
...
function buildChannelContext(
  ...
  const todoBlock = board && board.todos.length > 0 ? todoBoardContextBlock(board.todos) : "";
...
export function buildInteractionHandler(
  ...
  return async (d: any) => {
    ...
    const opt = Array.isArray(d.data?.options)
      ? d.data.options.find((o: any) => ["question", "mode", "instructions", "name", "scope"].includes(o?.name))
      : undefined;
...
function startCompact(pending: { ch: ChannelConfig; instructions?: string }, ctx: ExtensionContext): string | null {
  ...
  const report = (text: string) => { sendDiscordMessage(ch, text).catch(() => {}); };
  try {
    ctx.compact({
    ...
    case "todos": {
      ...
      if ((arg || "").trim().toLowerCase() === "all") {
        ...
        const parts = listBoards()
          ...
          .map(b => `▤ ${names.get(b.channelId) || b.channelId} · ${openCount(b.todos)} open\n${b.todos.map(todoLine).join("\n")}`);
        return { immediate: parts.length > 0 ? parts.join("\n\n") : "no open todos" };
      }
      const board = loadBoard(ch.id);
      if (!board || board.todos.length === 0) return { immediate: "no open todos" };
 ...
 * Exported for tests.
 */
export async function syncTodoBoard(ch: ChannelConfig, board: TodoBoard): Promise<void> {
  try {
    ...
    if (board.boardMessageId) {
      ...
      if (!r.success) console.error(`[todos] board edit failed: ${sanitizeSensitiveText(r.error || "")}`);
      return;
    }
    ...
    if (r.success && r.messageId) {
      ...
      console.error(`[todos] board post failed: ${sanitizeSensitiveText(r.error || "")}`);
    }
  } catch (e) {
...
export function registerTodoTool(pi: ExtensionAPI, channels: ChannelConfig[]): void {
  pi.registerTool({
    name: "todo",
    label: "Todo",
    description: TODO_TOOL_DESCRIPTION,
    promptSnippet: "Maintain the channel todo board (send the FULL updated list every time)",
    promptGuidelines: [
      "Call todo with the complete updated list whenever a task's status changes; empty list clears the board.",
    ],
    parameters: Type.Object({
      todos: Type.Array(Type.Object({
        content: Type.String({ description: "What to do (specific and actionable)" }),
        status: Type.Union([
      ...
      }), { maxItems: 50, description: "The COMPLETE updated list (empty array clears the board)" }),
    }),
    async execute(_toolCallId, params: { todos: any[] }, _signal, _onUpdate, ctx: ExtensionContext) {
      const active = lastActiveChannel && lastActiveChannel.type === "discord" ? lastActiveChannel : null;
      const ch = active || getDefaultChannel(channels);
      if (!ch || ch.type !== "discord") {
        return { content: [{ type: "text", text: "No Discord channel available" }], details: {} };
      ...
      const board: TodoBoard = {
        ...
        ...(prev?.boardMessageId ? { boardMessageId: prev.boardMessageId } : {}),
      };
      let stateNote = "";
      try {
        saveBoard(board);
      ...
      return { content: [{ type: "text", text: text + stateNote }], details: {} };
    },
  });
}
...
export async function handleInbound(pi: ExtensionAPI, msg: ChannelMessage, ctx: ExtensionContext, forceDirect = false) {
  ...
  if (isBgCallbackBody(rawBody)) {
    ...
    if (lines.length > 0) {
      try {
        const board = loadBoard(ch.id) ?? { channelId: ch.id, todos: [], updatedAt: "" };
        board.todos = mergeTodoLines(board.todos, lines);
        board.updatedAt = new Date().toISOString();
        saveBoard(board);
        await syncTodoBoard(ch, board);
      } catch (e) {
        console.error(`[todos] worker intake failed: ${sanitizeUnknownValue(e)}`);
      }
    }
  }
  ...
  const runActive = forceDirect ? false : (!ctx.isIdle() || ctx.hasPendingMessages?.());
