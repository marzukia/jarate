 ...
 *
 * One LLM tool: `send-file` uploads local file(s) to the active channel.
 *
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    // close the activity block (stays up, shows the finished run)
...
async function runChannelCommand(
  ...
  switch (name.toLowerCase()) {
    case "stop": {
      // Drop this channel's burst buffer: the flush timer would otherwise
      // fire within ~2.5s and start a new run with the pre-stop messages
      // (A4). Steer messages already queued in pi are a known upstream
      // gap — the extension context exposes hasPendingMessages() but no
      // clearQueue(), so they land at the next run's first turn boundary.
      // Needs pi to expose session.clearQueue() or abort({clearQueues:true}).
      const buf = burstBuffers.get(ch.id);
      if (buf) {
        if (buf.timer) clearTimeout(buf.timer);
        burstBuffers.delete(ch.id);
      }
      if (!ctx.isIdle()) {
        userStoppedRun = true;
        ctx.abort();
...
export async function handleInbound(
  ...
  // While a run is active, buffer rapid-fire messages and forward them as
  // one combined steer after a short quiet window.
  ...
  if (runActive) {
    ...
    if (!buf.timer) {
      buf.timer = setTimeout(() => {
        const b = burstBuffers.get(msg.channelId);
