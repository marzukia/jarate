...
import { cli as experimentalCli } from "./cli/experimental/cli.js";
...
import { runClient } from "./experimental/client.js";
import { runClientTui } from "./experimental/client-tui.js";
import { startForegroundServer } from "./experimental/server.js";
...
async function waitForTermination(serverClosed) {
...
async function runExperimentalServerCommand(command) {
    let previousRelayStatus = "";
    let relayOutputReady = false;
    let pendingRelayStatus;
    const reportRelayStatus = (status) => {
        const description = status.status === "connected"
            ? "connected"
            : status.status === "not_authenticated"
                ...
                : status.status === "retrying"
                    ...
                    : "connecting";
        if (description === previousRelayStatus || status.status === "connecting")
            return;
    ...
    const runtime = await startForegroundServer({
        ...
        onRelayStatus(status) {
            if (relayOutputReady)
                reportRelayStatus(status);
            else
                pendingRelayStatus = status;
        },
    });
    console.log(`Server: ${runtime.serverId}`);
    console.log(`Socket: ${runtime.socketPath}`);
    relayOutputReady = true;
    if (pendingRelayStatus !== undefined)
        reportRelayStatus(pendingRelayStatus);
    try {
...
async function runClientCommand(command) {
    ...
    for (const session of result.sessions)
        console.log(`${session.serverId}\t${session.sessionId}`);
}
async function runExperimentalCommand(args) {
    if (!areExperimentalFeaturesEnabled() || (args[0] !== "server" && args[0] !== "client"))
        return false;
    try {
