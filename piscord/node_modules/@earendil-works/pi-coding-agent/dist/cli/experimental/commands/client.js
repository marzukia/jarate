import { Command, flagOption, stringOption } from "../command.js";
import { authTokenFileOption, authTokenOption, connectOption, parseAuth, unsupportedOptions, } from "../command-options.js";
...
export const clientCommand = new Command("client")
    ...
    const prompt = promptArgs.length === 1 &&
    ...
    const sessionSelectionErrors = [sessionId !== undefined, shouldContinue, shouldResume].filter(Boolean).length > 1
        ? ["--session-id, --continue, and --resume are mutually exclusive"]
        : [];
    const unsupportedErrors = input.remainingArgs.length === 0 || prompt !== undefined ? [] : unsupportedOptions("client", input);
