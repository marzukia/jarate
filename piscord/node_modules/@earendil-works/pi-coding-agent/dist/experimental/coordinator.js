...
import { consumeInternalProcessRole, encodeControlLine, isDirectInternalProcessEntry, MAX_CONTROL_LINE_BYTES, spawnInternalProcess, } from "./process.js";
...
export class CoordinatorConnection {
    ...
    async connect() {
        if (this.#socket)
            throw new Error("Coordinator server is already connected");
    ...
    async #write(message) {
        if (!this.#registered || !this.#socket || this.#closed)
            throw new Error("Coordinator server is not connected");
    ...
    #markReplaced() {
        if (this.#replacedValue)
            return;
        this.#replacedValue = true;
        this.#resolveReplaced();
    }
}
export async function ensureCoordinator(publicPath, controlPath) {
    const existing = await tryConnect(controlPath);
    if (existing)
        return { close: () => existing.destroy() };
    ...
    while (true) {
        ...
        if (Date.now() >= deadline)
            throw new Error("Timed out waiting for coordinator startup");
        await new Promise((resolve) => setTimeout(resolve, COORDINATOR_RETRY_MS));
    }
}
...
function attachJsonLineReader(socket, onMessage) {
    let buffered = "";
...
export async function runCoordinatorProcess(args) {
    if (running)
        throw new Error("Coordinator process is already running");
...
function acceptControlConnection(socket) {
    if (shuttingDown) {
        socket.destroy();
        return;
    }
    ...
    attachRoutedLineReader(socket, (message) => {
        ...
        if (server) {
            if (server === currentServer)
                handleRoutedMessage("server", message);
            return;
        }
    ...
    const disconnect = () => {
        ...
        if (peer && peers.get(peer.peerId) === peer) {
            ...
            if (currentServer)
                writeRoutedLine(currentServer.socket, { type: "peer_disconnected", peerId: peer.peerId });
        }
        checkEmpty();
    };
...
function registerServer(socket, message) {
    if (message.protocol !== COORDINATOR_PROTOCOL_VERSION)
...
function registerPeer(socket, message) {
    ...
    if (peerId === "server" || peers.has(peerId))
...
function notifyPeers(message) {
    for (const peer of peers.values())
        writeRoutedLine(peer.socket, message);
}
function handleRoutedMessage(from, message) {
    if (message.type === "send") {
        ...
        if (target)
            writeRoutedLine(target, { type: "message", from, payload: message.payload });
        return;
    }
    if (message.type === "broadcast") {
        if (from !== "server")
            throw new Error("Only the current server may broadcast");
        for (const peer of peers.values()) {
...
function acceptPublicConnection(client) {
...
function closePublicConnections() {
...
function scheduleEmptyShutdown(delayMs) {
    if (emptyTimer || shuttingDown)
        return;
...
function cancelEmptyShutdown() {
    if (!emptyTimer)
        return;
    clearTimeout(emptyTimer);
    emptyTimer = undefined;
}
async function shutdownCoordinator() {
    if (shuttingDown)
        return;
    shuttingDown = true;
    cancelEmptyShutdown();
...
function writeRoutedLine(socket, message) {
    if (!socket.destroyed)
        socket.write(encodeControlLine(message));
}
function attachRoutedLineReader(socket, onMessage) {
    let buffered = "";
    socket.setEncoding("utf8");
    socket.on("data", (chunk) => {
        buffered += chunk;
        ...
        while (true) {
            ...
            try {
                ...
                if (typeof message?.type !== "string")
                    throw new Error("Coordinator message must have a type");
                onMessage(message);
            }
            catch (error) {
...
async function restrictSocket(path) {
...
async function removeStaleSocket(path) {
    try {
        ...
        if (!stats.isSocket())
            throw new Error(`Coordinator path is not a socket: ${path}`);
    }
    catch (error) {
    ...
    if (live)
        throw new Error(`Coordinator socket is already active: ${path}`);
    await cleanupSocket(path);
}
async function cleanupSocket(path) {
    if (process.platform === "win32")
        return;
