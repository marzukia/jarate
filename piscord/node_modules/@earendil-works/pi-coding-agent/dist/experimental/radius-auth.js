...
export class RadiusRelayAuthResolver {
    ...
    #gateway;
    #modelRuntime;
    constructor(input, gateway = process.env[ENV_RADIUS_GATEWAY] ?? DEFAULT_RADIUS_GATEWAY) {
        this.#input = input;
    ...
    async resolve(options) {
        ...
        if (process.env.PI_OFFLINE !== undefined) {
            if (options.required)
                throw new Error("Radius relay connections are unavailable in offline mode");
            return undefined;
        }
        const explicit = await this.#explicitToken(options.signal);
        if (explicit !== undefined)
            return { gateway: this.#gateway, token: explicit };
        this.#modelRuntime ??= ModelRuntime.create({ refreshOnCreate: false, allowModelNetwork: false });
        const runtime = await this.#modelRuntime;
        options.signal?.throwIfAborted();
        const token = getAuthCredential(await runtime.getAuth("radius", { minOAuthValidityMs: 5 * 60_000, signal: options.signal }));
    ...
    async #explicitToken(signal) {
        if (this.#input === undefined)
            return undefined;
        const value = this.#input.type === "token"
            ? this.#input.token
            : await readFile(resolvePath(this.#input.path), { encoding: "utf8", signal });
        const token = value.trim();
        if (token.length === 0)
            throw new Error("Radius authentication token must not be empty");
        return token;
    }
}
//# sourceMappingURL=radius-auth.js.map