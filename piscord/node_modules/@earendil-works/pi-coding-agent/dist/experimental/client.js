...
import { activateBuiltinClientServices, openClientRuntime } from "./client-runtime.js";
/** Discover servers, then list Sessions, attach to one, or create one for a prompt. */
export async function runClient(command, options = {}) {
    ...
    try {
        ...
        if (sessionId === undefined && command.prompt === undefined) {
            return {
                kind: "list",
                sessions: discovered
                    .flatMap(({ route, directory }) => directory.state.value.sessions.map(({ sessionId }) => ({ serverId: route.serverId, sessionId })))
                    .sort((left, right) => left.serverId.localeCompare(right.serverId) || left.sessionId.localeCompare(right.sessionId)),
            };
        }
        let match;
        if (sessionId === undefined) {
            ...
            const matches = discovered.filter((candidate) => candidate.directory.state.value.sessions.some(({ sessionId }) => sessionId === selectedSessionId));
            if (matches.length > 1) {
