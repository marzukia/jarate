...
import { FACET_BUNDLE_MANIFEST_FILE, readFacetBundleArtifact, } from "@earendil-works/chord/node";
...
export async function restoreServerPluginPackageProfile(directory, serverId, configuredPackagePaths) {
    ...
    if (packagePaths.length === 0)
        ...
        await writePluginPackageProfile(path, packagePaths);
    return packagePaths;
}
/** Read the explicit plugin selection stored for one durable Session. */
export function readSessionPluginPackageProfile(directory, serverId, sessionPath) {
...
export async function removeSessionPluginPackageProfile(directory, serverId, sessionPath) {
...
export function writeSessionPluginPackageProfile(directory, serverId, sessionPath, packagePaths) {
    return writePluginPackageProfile(sessionPluginProfilePath(directory, serverId, sessionPath), normalizePluginPackagePaths(packagePaths), sessionPath);
}
/** Create a serialized server-owned builder for one configured plugin package. */
export function createServerPluginPackage(directory, serverId, packagePath) {
    ...
    return {
        ...
        build() {
            ...
            buildTail = operation.then(() => undefined, () => undefined);
            return operation;
        },
    };
}
async function readPluginPackageProfile(path, allowEmpty, sessionPath) {
    let parsed;
    try {
    ...
    if (typeof parsed !== "object" ||
        ...
        Object.keys(parsed).some((key) => key !== "version" && key !== "packagePaths" && !(key === "sessionPath" && sessionPath !== undefined)) ||
        !("version" in parsed) ||
...
function writePluginPackageProfile(path, packagePaths, sessionPath) {
...
export function normalizePluginPackagePaths(packagePaths) {
    const normalized = packagePaths.map((packagePath) => {
        if (packagePath.length === 0)
            throw new Error("Plugin package path must not be empty");
