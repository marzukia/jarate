...
export class ProtocolTestClient {
    ...
    async requestService(target, call, id = `request-${++this.requestSequence}`) {
        const response = this.next((message) => message.type === "response" && message.id === id);
        await this.sendMessage({ type: "request", id, target, call: call });
        return (await response);
    }
    attach(serverId, sessionId) {
        return this.requestService({ serverId }, { serviceId: "pi.session-management", member: "attach", args: [sessionId] });
    }
    requestSessionService(serverId, sessionId, call, id) {
        const attachment = this.attachment;
        const target = attachment === undefined || attachment.sessionId !== sessionId
