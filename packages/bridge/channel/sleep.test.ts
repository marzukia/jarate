...
describe("sleep: parseWakeAt", () => {
  ...
  test("invalid ISO -> error", () => {
    expect(parseWakeAt({ until: "tomorrow" }, NOW).error).toContain(
      "invalid until",
    );
  });

  test("past until -> error", () => {
    expect(parseWakeAt({ until: "2026-09-09T15:00:00Z" }, NOW).error).toContain(
      "future",
    );
  });
  ...
  test("minutes over 30d cap -> error", () => {
    expect(
      parseWakeAt({ minutes: MAX_WAKE_MS / 60000 + 1 }, NOW).error,
    ).toContain("too large");
  });

  test("until within 30d cap is accepted; over the cap -> error", () => {
    const ok = parseWakeAt(
      { until: new Date(NOW + MAX_WAKE_MS - 1000).toISOString() },
      NOW,
    );
...
describe("sleep: state file", () => {
  ...
  test("stale claim re-due after CLAIM_TTL_MS (restart simulation: re-read the file)", () => {
    ...
    expect(
      dueWakes("ch1", NOW + CLAIM_TTL_MS + 1, tmp).map((w) => w.id),
    ).toEqual([a.id]);
  });
  ...
  test("cancelPendingWakes removes only PENDING wakes for the channel", () => {
    const pending = scheduleWake({
      channelId: "ch1",
      wakeAt: NOW + 60000,
      home: tmp,
      now: NOW,
    });
    const other = scheduleWake({
      channelId: "ch2",
      wakeAt: NOW + 60000,
      home: tmp,
      now: NOW,
    });
    const claimed = scheduleWake({
      channelId: "ch1",
      wakeAt: NOW + 120000,
      home: tmp,
      now: NOW,
