...
describe("formatContext", () => {
  test("exact frame, deterministic now (n=5 shows everything)", async () => {
    ...
    expect(formatContext(s, 5, now)).toBe(
      [
        ...
        "│  2 asst thinking     2K 3m",
        "│  3 tool toolResult   1K 5m",
        "│  4 asst toolCall      5 6m",
        "│  5 user text          3 0s",
        "├ totals",
        "│ user          3  (1)",
        "│ assistant    2K  (2)",
        "│ tool         4K  (2)",
        "└ biggest: tool toolResult   3K",
      ].join("\n"),
    );
  });
  ...
  test("biggest line worst case: assistant toolCall + 999K stays <= 32", () => {
    ...
    expect(biggest).toBe("└ biggest: asst toolCall    23K");
    expect(biggest.length).toBeLessThanOrEqual(32);
  });
...
describe("renderContext", () => {
  test("15 items: default top-10 order + /context 20 shows all 16", async () => {
    ...
    expect(def).toContain("┌ top 10/16 · total 300K");
    ...
    expect(wide).toContain("┌ top 16/16 · total 300K");
  ...
  test("fallback: sessionFile null -> findSessionFile(cwd)", async () => {
    ...
    expect(text).toContain("┌ top 2/2 · total 2K");
